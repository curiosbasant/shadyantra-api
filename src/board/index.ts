export { Builder, default as Board } from './Board';
export { default as BoardState } from './BoardState';
export { default as History } from './History';
export { AttackMove, default as Move } from './Movement';
export { CastleZone, default as Square, ForbiddenZone, SquareName, SQUARE_FLAGS, TruceZone, WarZone, MediateZone, Zone, ZoneName } from './Square';
import { Arthshastri, Ashvarohi, Gajarohi, Guptchar, Maharathi, Pyada, Rajendra, Rajrishi, Senapati } from './../pieces';
/* export const Yantra = {
  P: Pyada,
  C: Guptchar,
  G: Gajarohi,
  H: Ashvarohi,
  M: Maharathi,
  S: Senapati,
  A: Arthshastri,
  I: Rajendra,
  R: Rajrishi,
}; */
